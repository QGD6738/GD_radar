...
/*const*/ WORD  TableWidth[] =
{
      ...
      150,/*	87.0cm	~	95.7cm	*/
      150,/*	95.7cm	~	104.4cm	*/
      200,//150,/*	104.4cm	~	113.1cm	*/
      200,//120,/*	113.1cm	~	121.8cm	*/
      200,//120,/*	121.8cm	~	130.5cm	*/
      200,//120,/*	130.5cm	~	139.2cm	*/
      200,//120,/*	139.2cm	~	147.9cm	*/
      200,//120,/*	147.9cm	~	156.6cm	*/
      200,//120,/*	156.6cm	~	165.3cm */
      200,//120,/*	165.3cm	~	174.0cm */
};
...
const BYTE  TableHeight[] =
{
      ...
      19, /*	87.0cm	~	95.7cm	*/
      19, /*	95.7cm	~	104.4cm	*/
      19,//14, /*	104.4cm	~	113.1cm	*/
      19,//12, /*	113.1cm	~	121.8cm	*/
      45,//11, /*	121.8cm	~	130.5cm	*/
	  45,//10, /*	130.5cm	~	139.2cm	*/
	  45,//10, /*	139.2cm	~	147.9cm	*/
	  45,//10, /*	147.9cm	~	156.6cm	*/
	  45,//10, /*	156.6cm	~	165.3cm */
      45,//10, /*	165.3cm	~	174.0cm */
};
...
void DetObj_InitSelfCheckVar(void)
{
    ...
    for(i=0;i<SN_NUM;i++)
    {
         ...
         NearestData.bRingTimeFlag[i] = 0x0F;
    }
...
void DetObj_InitVariable(void)
{
	...
	DiagInfo.bErrCounter[DetInfo.bID] = 0;
	...
	for(i=0;i<SN_NUM;i++)
	{
		NearestData.bRingTimeFlag[i] = 0;
		NearestData.wUpdataDelay[i] = 0;
		NearestData.wRingTime[i] = 0xFFFF;
		NearestData.bRingTimeBounceFlag[i] = 0;
		NearestData.bEachNearestDistance[i] = 0xFF;
	}
...
//    if(NearestData.wUpdataDelay[DetInfo.bID] > 0)
//    {
//        NearestData.wUpdataDelay[DetInfo.bID]--;
//    }
	BYTE i;
	DetInfo.wDetRealTime++;
	for(i=0;i<ID_SUM;i++)
	{
		if(NearestData.wUpdataDelay[i] > 0)
		{
			NearestData.wUpdataDelay[i]--;
		}
	}
...
void DetObj_SendThe40KhzUltraSonic(void)
{
    volatile BYTE i;

    Cpu_DisableInterrupt();
    for(i=PULE_NUM;i>0;i--)             /* Send the 40KHz ultrasonic. */
    {
        DRIVE_PORT_1 = 0;
//        DRIVE_PORT_2 = 1;
//        Delay1us(6);
...
//        Delay1us(6);
    ...
    DRIVE_PORT_1 = 0;
//    DRIVE_PORT_2 = 0;
    RawData.wStartTime = PULE_NUM*25;
    Cpu_EnableInterrupt();
}
...
{
    if(DiagInfo.bErrCounter[DetInfo.bID] >  SN_ERR_THRESHOLD)
    {
...
{
	if(NearestData.bRingTimeFlag[DetInfo.bID] > 0)
    {
        ...
        {
            NearestData.bRingTimeFlag[DetInfo.bID] >>= 1;
            NearestData.wRingTime[DetInfo.bID] = CmpData.EPD[CmpData.bCurBfNumber].wRingTime;
            if(DiagInfo.bErrCounter[DetInfo.bID] > 0)
            {
                DiagInfo.bErrCounter[DetInfo.bID]--;
            }
        }
        ...
        {
            NearestData.bRingTimeFlag[DetInfo.bID] >>= 1;
            NearestData.wRingTime[DetInfo.bID] = RING_TIME_MID_VALUE;
        }
        else
        {
            if(DiagInfo.bErrCounter[DetInfo.bID] < 8)
            {
                DiagInfo.bErrCounter[DetInfo.bID]++;
            }
        }
    	if((NearestData.wRingTime[DetInfo.bID]<1300) && (NearestData.wRingTime[DetInfo.bID] > 500))
    		NearestData.wRingTime[DetInfo.bID] = RING_TIME_MID_VALUE;
    }
    else
    {
...
//	if((NearestData.wRingTime[DetInfo.bID]<1000) && (NearestData.wRingTime[DetInfo.bID] > 500))
//		NearestData.wRingTime[DetInfo.bID] = 1000;
    DetObj_DiagnosticTheSensor();
}
...
void DetObj_CalSampNoise(void)
{
    ...
    //	 if(AnsData.bDetSampThre >= ((16<<0)+DETECT_NOISE_BASE_VOL))
...
//            DetObj_SendThe40KhzUltraSonic();
      ...
      case DET_OBJ_WAIT_ECHO:
            ...
            else
            {
                ...
                //test
//                if(CmpData.EPD[CmpData.bCurBfNumber].wRingTime < 1000)
//                {
//                	CmpData.EPD[CmpData.bCurBfNumber].wRingTime = CmpData.EPD[CmpData.bCurBfNumber].wRingTime;
//                }
//                if(NearestData.wRingTime[DetInfo.bID] < 1000)
//                {
//                	NearestData.wRingTime[DetInfo.bID] = NearestData.wRingTime[DetInfo.bID];
//                }
            }
            break;
...
void DetObj_HoldTheObstaclestate(void)
{
    ...
    NearestData.bRingTimeBounceFlag[DetInfo.bID] = FALSE;
    CmpData.EPD[CmpData.bCurBfNumber].bDataNum = 1;
    NearestData.wUpdataDelay[DetInfo.bID] = DETECT_DELAY_UPDATE_HIGH;
    CmpData.CCD.wRingTime = NearestData.wRingTime[DetInfo.bID];
    DiagInfo.bKeepFlag = TRUE;
//    CmpData.CCD.bObsDistance[0] = NearestData.bEachNearestDistance[DetInfo.bID];
    CmpData.CCD.bObsDistance[0] = NO_OBJ;
    for(i=1; i<OBJ_NUM; i++)
    {
        CmpData.CCD.bObsDistance[i] = NO_OBJ;
    }
...
void DetObj_GenerateTheNearestObstacle(void)
{
    ...
    if(bDis > MAX_DETECT_DISTANCE)
    {
           ...
           )
        {
            ...
            {
                NearestData.wUpdataDelay[DetInfo.bID]= DETECT_DELAY_UPDATE_MID;
            }
            else
            {
                NearestData.wUpdataDelay[DetInfo.bID]= DETECT_DELAY_UPDATE_LOW;
            }
            NearestData.bEachNearestDistance[DetInfo.bID] = OBSTACLE_IN_CONTINUOUS_DISTANCE_DELAY;
        }*/
...
void DetObj_ProcessTheRingTime(void)
{
    ...
    if(NearestData.bRingTimeBounceFlag[DetInfo.bID] == FALSE)
    {
        if(CmpData.CCD.wRingTime > NearestData.wRingTime[DetInfo.bID])
        {
            /* Compute the difference between the ring time saved and this time detected. */
            wRingTimeDiff = CmpData.CCD.wRingTime - NearestData.wRingTime[DetInfo.bID];
...
void DetObj_CompareTheData(void)
{
    ...
    if(CmpData.bFirstDataFlag == TRUE)
    {
        ...
        if(NearestData.bRingTimeFlag[DetInfo.bID] == 0)
        {
            if(CmpData.CCD.bDataNum ==0)
            {
                if(CmpData.EPD[CmpData.bCurBfNumber].wRingTime < (NearestData.wRingTime[DetInfo.bID] + RENEW_RING_TIME_THRESHOLD))
                {
    ...
    else
    {
        ...
        else
        {
            ...
            NearestData.bRingTimeBounceFlag[DetInfo.bID] = TRUE;
        ...
        if(CmpData.CCD.bDataNum == 0 &&
           CmpData.CCD.wRingTime < (NearestData.wRingTime[DetInfo.bID] + RING_TIME_CHANGE_THRESHOLD))
        {
