...
void ADC_PowerSamplingInit(void)
{
	...
	ADC0LTH			= 0x00;
	ADC0LTL			= 0x00;
	ADC0MX			= 0x0E; //Selects the channel P0.0-P1.6 == 00000-01110
...
void ADC_ReadID(void)
{
	...
	//Judge ID code...
	if(ADC.Power_SampleValue >= (ADC.ID_SampleValue-RML_ADSAMPLE_DEC))
	{
		bRadar_PoweFlag = TRUE;
		wPowerValue = ADC.Power_SampleValue;
	}
	else
	{
		bRadar_PoweFlag = FALSE;
		wPowerValue = ADC.ID_SampleValue;
	}

    RL_MIN_VALUE = 0;      // GND
	RL_MAX_VALUE = 100;

	RML_MIN_VALUE = (WORD)(((DWORD)wPowerValue*75 )/100);//(WORD)(wPowerValue-20); // +12V
	RML_MAX_VALUE = (WORD)(((DWORD)wPowerValue*117)/100);

	RR_MIN_VALUE = 100; // NC
	RR_MAX_VALUE = (WORD)(((DWORD)wPowerValue*75 )/100);

    FL_MIN_VALUE = 0;	// GND
    FL_MAX_VALUE = 100;

    FR_MIN_VALUE = 100; // NC
    FR_MAX_VALUE = (WORD)(((DWORD)wPowerValue*83 )/100);//WORD)((wPowerValue*6)/8-24+100);
	...
	{
		if(ADC.Power_SampleValue >= FL_MIN_VALUE && ADC.Power_SampleValue < FL_MAX_VALUE)
		{
			DetInfo.bID  = ID_FL;
		}
		if(ADC.Power_SampleValue > FR_MIN_VALUE && ADC.Power_SampleValue < FR_MAX_VALUE)
		{
			DetInfo.bID  = ID_FR;
		}
	}
