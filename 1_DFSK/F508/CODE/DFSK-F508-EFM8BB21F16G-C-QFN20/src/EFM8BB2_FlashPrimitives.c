...
void Flash_WriteVechileMode(uint8_t mode)
{
	FLASH_PageErase(VechileMode_START_ADDRESS);
	FLASH_ByteWrite(VechileMode_ADDR, mode);
}

uint8_t Flash_ReadVechileMode  (void)
{
	uint8_t temp_byte;
	temp_byte = FLASH_ByteRead(VechileMode_ADDR);
