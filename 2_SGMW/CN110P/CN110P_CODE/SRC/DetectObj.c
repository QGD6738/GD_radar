...
void DetObj_RealTimeClock(void)
{
    ...
    if( (Cmd.CurSta > CMD_SEND_SELF_DATA_TO_CAR) && (DetInfo.bID == MASTER_SENSOR) )
    {
        Spk_DrvTheSpeaker();/* Chime the object state. */ 
    }
}
...
void DetObj_SendThe58KhzUltraSonic(void)
{
    ...
    PC_ODR = IO;
    RawData.wStartTime = PULE_NUM*17;//17
    Cpu_EnableInterrupt();
}
...
BYTE GetDistance(WORD ObsTime,WORD EchoWidth,BYTE VolHeight)
{
    ...
    Redundancy = 0;
    if(NearestData.bEachNearestDistance[DetInfo.bID] < MAX_DETECT_DISTANCE)
    {
    	Redundancy = REDENDANCY_VALUE;
    }
    
    Index = ObsTime >> 9;
    if(EchoWidth >= TableWidth[Index] && VolHeight >= (TableHeight[Index]-Redundancy))
    {
        ObsTime >>= 7;
        ret = (ObsTime*557) >> 8;//557
    }
    return ret;
}
    ...
    for(i=0; i<RawData.wRawDataIndex;i++)
    {
        ...
        else if(RawData.bRawDataBuffer[i] < (AnsData.bObsSigVolHeight>>OBSTACLE_EXIT_VOL_SHIFT))
        {
            if(AnsData.bObsSigStartFlag == TRUE)
            {
                ...
                {
                    AnsData.bObsSigStartFlag = FALSE;
                    if(AnsData.bObsSigVolHeight > ((AnsData.bDetSampThre + DETECT_NOISE_BASE_VOL)<<1) 
		       && AnsData.wObsSigTimeWidth > DETECT_NOISE_WIDTH )
                    {
    ...
    switch(DetInfo.bDetState)
    {
      ...
      case DET_OBJ_HANDLE_NOISE:
            ...
            if(
              ...
              //||  CmpData.Ns[CmpData.bCurBfNumber].bNoiseRaisedTimes>= 5 
              ||  CmpData.Ns[CmpData.bCurBfNumber].dObsIntegralArea >= ((DWORD)110*DETECT_MAX_DISTANCE_150CM*1000/COUNT_TIME_UNIT)
              ||  CmpData.Ns[CmpData.bCurBfNumber].dNseIntegralArea >= ((DWORD)28*DETECT_MAX_NOISE_TIME*1000/COUNT_TIME_UNIT)
                  )
               )
            {
                DetInfo.bDetTimes = 0;
      ...
      case DET_OBJ_GENERATE_RETURN:
            ...
            {
                DetInfo.bDetTimes--;
                DetInfo.bDetNoiseDelayTime = DetectInterval[CmpData.Ns[CmpData.bCurBfNumber].dObsIntegralArea%INTERVAL_LEN];
